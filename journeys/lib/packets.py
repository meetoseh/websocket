...
the journey watch websocket, approximately in the order they would normally
be sent.
"""
...
class AuthRequestPacketData(BaseModel):
    journey_uid: str = Field(
        description="The UID of the journey to watch", min_length=1, max_length=255
    )
    jwt: str = Field(
        description="The JWT which proves they are allowed to watch the journey",
        min_length=2,
    )
    bandwidth: int = Field(
    ...
    lookback: float = Field(
        description=(
            ...
            journey time of 11 seconds, it will still receive live events from a
            journey time of 6 seconds, but not from 5 seconds.
            """
        ),
        ge=0,
        le=10,
    ...
    lookahead: float = Field(
        description=(
            ...
            journey time of 11 seconds, it will still receive live events from a
            journey time of 16 seconds, but not from 17 seconds.
            """
        ),
        ge=0,
        le=10,
...
class SyncResponsePacketData(BaseModel):
    receive_timestamp: float = Field(
        description=(
            "The journey time that the client received the sync request at, in seconds "
            "from the start. The client can start their journey time at a negative "
    ...
    transmit_timestamp: float = Field(
        description=(
            "The journey time that the client sent back the sync response at, in seconds "
            "from the start. The client can start their journey time at a negative "
...
class EventBatchPacketDataItem(BaseModel):
    ...
    journey_time: float = Field(
        description=(
            ...
            "the event is prior to the clients journey time, the client should present "
...
class LatencyDetectionPacketData(BaseModel):
    expected_receive_journey_time: float = Field(
        description=(
            "What the server expects the clients journey time "
...
accepted, which begins the process of the server syncing a journey time clock with
the client.
"""
...
actual journey time as soon as possible, and then the future ones at the
appropriate journey time.
"""
...
client can detect network interruptions by comparing the journey time it was at
when it received the packet with the journey time it expects to be at when it
